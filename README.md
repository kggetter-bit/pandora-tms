...
const THAILAND_GEOJSON = { type: "Feature", geometry: { type: "Polygon", coordinates: [THAILAND_BOUNDARY] } };

function RealThailandMap({ width = 400, height = 520, children }) {
  const projection = d3.geoMercator().fitSize([width, height], THAILAND_GEOJSON);
  const pathGen = d3.geoPath(projection);
  const project = (lat, lng) => { const [x, y] = projection([lng, lat]); return { x, y }; };

  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" style={{ background: "#EAF4FB", borderRadius: 10 }}>
      <path d={pathGen(THAILAND_GEOJSON)} fill="#DCEFE1" stroke="var(--teal)" strokeWidth="1.3" />
      {children(project)}
    </svg>
  );
}
  ...
  return (
    <>
      ...
      <div className="card" style={{ padding: 10, marginBottom: 20 }}>
        <GoogleMapEmbed stops={zoneEntries.map(([zone]) => ZONE_COORDS[zone])} />
